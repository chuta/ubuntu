...
function newDocumentUrl(partnershipId: string, organizationId: string, ai?: boolean) {
    ...
    <div className="rounded-xl border border-gray-200 bg-white">
      <div className="flex flex-col gap-3 border-b border-gray-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <Link href={newDocumentUrl(partnershipId, organizationId)}>
            <Button size="sm" variant="outline">
              <Plus className="mr-1.5 h-4 w-4" />
              New Document
            </Button>
          </Link>
          <Link href={newDocumentUrl(partnershipId, organizationId, true)}>
            <Button size="sm">
      ...
      {documents.length === 0 ? (
        <p className="p-8 text-center text-sm text-gray-500">
          No documents linked to this partnership yet.
        </p>
      ) : (
